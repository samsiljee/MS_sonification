...
for(i in ms_header$seqNum) {
  if(ms_header$msOrder == "ms1") {
    double_plot(
  spectrum_left = as.data.frame(ms_peaks[[i]]),
  spectrum_right = data.frame(mz = min(ms_peaks[[i]]$mz), intensity = 1))
  } else {
   double_plot(
  ...
  precursor_mz = ms_header$percusormz[ms_header$seqNum == i],
  precursor_intensity = ms_header$percusorIntensity[ms_header$seqNum == i])
  }
